...
for i in $(seq 1 $COUNT); do
  ...
  PR_URL=$(gh pr create --title "Pull Shark $TS-$i" --body "Pull Shark automation for badge-tool")
  gh pr merge "$PR_URL" --merge --admin
  git checkout main
done
echo "Success! Checked your profile for Pull Shark: https://github.com/detritivore200?tab=achievements"
